...
void Game::CreateBasicGeometry()
{
	...
	std::shared_ptr<Entity> eCube = std::make_shared<Entity>(cube);
	eCube->GetTransform()->SetPosition(-4, 0, 0);

	std::shared_ptr<Entity> eHelix = std::make_shared<Entity>(helix);
	eHelix->GetTransform()->SetPosition(4, 0, 0);

	std::shared_ptr<Entity> eTorus = std::make_shared<Entity>(torus);
